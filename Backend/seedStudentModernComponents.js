...
const studentModernTemplate = {
  ...
  role: 'student',
  category: 'modern',
  thumbnail: 'https://firebasestorage.googleapis.com/v0/b/websitebuilder-b1f96.firebasestorage.app/o/templates%2Fstudent_modern1.png?alt=media&token=8b7f8803-497b-4542-a08c-4e4b57b10caf',
  availableComponents: [
...
const seedStudentModernComponents = async () => {
  try {
    await dbCon();
    
    // Clear existing student modern template
    await Template.deleteOne({ key: studentModernTemplate.key });
    console.log('Cleared existing student modern template');
    
    // Insert template
    const template = await Template.create(studentModernTemplate);
    console.log(`Template created: ${template.name} (${template.key})`);
    
    // Clear existing student modern components
    await Component.deleteMany({ 
      'customFields.template': 'student', 
      'customFields.variant': 'modern' 
    });
    console.log('Cleared existing student modern components');
    
    // Insert new components
    const result = await Component.insertMany(studentModernComponents);
    
    console.log(`Successfully inserted ${result.length} student modern components:`);
    result.forEach(component => {
      console.log(`- ${component.customName} (${component.type})`);
    });
    
    console.log('\nStudent modern template and components seeded successfully!');
    
  } catch (error) {
    console.error('Error seeding student modern template and components:', error);
  } finally {
