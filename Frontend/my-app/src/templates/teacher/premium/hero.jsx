...
const TeacherPremiumHero = ({ content = component.content, style = component.style }) => (
  <motion.section
    initial={{ opacity: 0, y: 40 }}
    animate={{ opacity: 1, y: 0 }}
    transition={{ duration: 0.8 }}
    className={`${style.bg} w-full py-12 sm:py-16 lg:py-20 px-4 sm:px-8 lg:px-16 relative overflow-hidden`}
  >
    {/* Background Pattern */}
      ...
      <div className="flex-1 flex flex-col items-center lg:items-start text-center lg:text-left gap-6 lg:gap-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6 w-full max-w-lg lg:max-w-none">
          {content.stats.map((stat, idx) => (
            <motion.div 
              key={idx} 
              className="text-center bg-white rounded-xl sm:rounded-2xl p-3 sm:p-4 border border-purple-200 shadow-lg"
      ...
      <div className="flex-1 flex justify-center items-center w-full lg:w-auto">
        ...
        >
          ...
          <img 
            ...
            className="relative w-full rounded-2xl lg:rounded-3xl shadow-2xl object-cover border-2 sm:border-4 border-purple-200"
          />
        </motion.div>
      </div>
    </div>
...
export default TeacherPremiumHero; 