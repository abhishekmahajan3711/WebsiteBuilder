...
const TeacherBasicPrivateClassTimings = ({ content , style }) => (
  <motion.section
    ...
    <ul className="mb-2">
      {content.items.map((slot, idx) => (
        <li key={idx} className="flex justify-between mb-1">
          <span className={style.day}>{slot.day}</span>
          <span className={style.time}>{slot.time}</span>
        </li>
      ))}
    </ul>
...
export default TeacherBasicPrivateClassTimings; 