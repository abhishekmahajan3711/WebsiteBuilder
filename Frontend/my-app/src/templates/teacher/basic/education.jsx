...
const TeacherBasicEducation = ({ content , style }) => (
  <motion.section
    ...
    <ul>
      {content.items.map((item, idx) => (
        <li key={idx} className="mb-2">
          <div className={`font-bold ${style.degree}`}>{item.degree}</div>
          <div className={style.college}>{item.college}</div>
...
export default TeacherBasicEducation; 